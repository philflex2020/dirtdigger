 ...
 * update Phil Wilshire now for Linux C++
 * 6/15/2019 ( 20 years ON !!! Way to go Linux)
 */
/* This program compiles for Linux //Sparc Solaris 2.6.
 * To compile for Linux:
 *  NAH 1) Comment out the #include <pthread.h> line.
 *  NAH 2) Comment out the line that defines the variable newthread.
 *  NAH 3) Comment out the two lines that run pthread_create().
 *  NAH 4) Uncomment the line that runs accept_request().
 *  YEA 5) Remove -lsocket from the Makefile.
 * the pthread task is  void * task(void *) under linux
 * we'll C++ this up over time
 */
#include <stdio.h>
...
void not_found(int client, const char * file)
{
 ...
 send(client, buf, strlen(buf), 0);
 sprintf(buf, "your request because the resource [%s] \r\n", file);
 send(client, buf, strlen(buf), 0);
