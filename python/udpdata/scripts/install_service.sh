...
if ! ( type -P pip3 > /dev/null ) ; then
	printf "udpdata requires Python 3 pip\n"
...
systemctl enable --no-pager udpdata.service
systemctl restart --no-pager udpdata.service
systemctl status --no-pager udpdata.service
