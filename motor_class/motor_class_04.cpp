...
// big move in this one to to get an input via the polling interface
//
#include <iostream>
#include <fstream>
#include <string.h>
...
// upto now the input comes in from stdin

// console formatter not used here
string readConsole(int fd)
{
  string res = "";
  char buf[1024];
  ...
  if (rc > 0) {
    buf[rc] = 0;
    printf(" read %d bytes last byte %x\n", rc, buf[rc-1]);
    if(buf[rc-1] == 0xa) buf[rc-1] =  0; 
    if(buf[strlen(buf)] == '\r') buf[strlen(buf)] =  0; 
    res=buf;
  }
  return res;
}
...
// the server will start up a socket and create clients from incoming connections to those sockets
