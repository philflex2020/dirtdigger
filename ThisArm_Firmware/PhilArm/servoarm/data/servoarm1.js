...
connection.onmessage = function (e) {  
    //if(typeOf e.data === String ){
         ...
         var Rcol = jsonObject.R;
         var Gcol = jsonObject.G;
         var Bcol = jsonObject.B;

         document.getElementById('r').value = Rcol;
         document.getElementById('g').value = Gcol;
         document.getElementById('b').value = Bcol;
		
      console.log('Received data string R:' +Rcol+ ' G:' + Gcol + ' B:' + Bcol );
   //}
...
function moveSrs(angle1, angle2, angle3, angle4, angle5, angle6)
{
	...
	//send command
    var armstr =  "srs": angle1 + " " + angle2 + " " + angle3 + " " + angle4 + " " + angle5 + " " + angle6 
    console.log('ARM: ' + armstr); 
    connection.send(armstr);
