...
<script>
  ...
  var aData = '{"anin1":{"label":"motor speed","value":"220"},"anin2":{"label":"motor range","value":"520"},"anin3":{"label":"aux1","value":"20"},"anin4":{"label":"aux2","value":"220"} }';
...
window.onload = function(e)
  ...
  function getFormData(){
    var str = getFormStr();
    var sstr = "{\"cmd\":\"filewrite\",\"name\":\"/config.json\",\"data\":";
    sstr += str;
    sstr += "}";
  ...
  function setAnins() {
    ...
    var jq = JSON.parse(aData); //(aData);
    console.log(" running setAnins");
    console.log(jq);
    for (x in jq) {
      console.log(x);
      ain = document.getElementById(x);
      ainlab = document.getElementById(x+'lab');
      ainout = document.getElementById(x+'out');
      console.log(jq[x]);
      jqi = jq[x];
      if(ainlab) {
        console.log(ainlab);
        ainlab.innerHTML = jqi["label"];        
      }
      if(ain) {
        ain.value = jqi["value"];        
      }
      if(ainout) {
        ainout.innerHTML = jqi["value"];        
      }
      for (y in jqi) {
           console.log(y);
           console.log(jqi[y]);
...
<body>
    ...
    <table>
     <tr>
      ...
      <td><button onclick="setAnins()">Anins</button> <br /></td>
    ...
    <table id="analogs">
     <tr>
       <td><label id="anin1lab">Analog input 1:</label></td>
       <td><input type="range" class="range2" id="anin1" name="ain1" min="0" max="1024" oninput="myAnin('anin1','anin1out')"></td>
       <td><button type="button" class="button2" onclick="sendCommand('1','0')">Set</button> </td>
       <td><output id="anin1out"></output></td>
       
     </tr><tr>
       <td><label id ="anin2lab">Analog input 2:</label></td>
       <td><input type="range" class="range2" id="anin2" name="ain2" min="0" max="1024" oninput="myAnin('anin2','anin2out')"></td>
       <td><button type="button" class="button2" onclick="sendCommand('1','0')">Set</button> </td>
       <td><output id="anin2out"></output></td>
     </tr>
    </table>
    <table id="table0">
     <tr>
