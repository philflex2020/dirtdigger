...
#define MAX_IDS 128
#define MAX_FOUND 32
int timeGap = 100;     // time between slots
int idCount[MAX_IDS];
int baseFound[MAX_IDS];
int foundIds [MAX_IDS];
long baseTime[MAX_IDS];
int accepted[MAX_IDS];
...
// how this works
// at stat we put myid at 31
// like at the end of all othes
// we scam for any signals and mark any we find and calculate a base time
// we only care about the base time if the othe one has found us
// there may be others at id 31 so we have to deal with that
//
// if we find any new ids we immediately respond with a ping back the same id and our id
// we also set our base time to the estimate from the incoming signal 
// several units may also ping back so that signal may be lost.
// we keep pinging back with random delay until we get a response.
// if not lost the othe unit will ping back with our id and its id. 
// that seals the transcation that unit is marked as found.
...
int inIdMask(int id, int mask)
{
  if ( ( mask == 0 ) || ( id >= MAX_IDS))
    return 0;
  return (mask && (1<<id));
}

}
int pingResponse( int srcid, int destid)
{
       //setAccepted(srcid);
       accepted[srcid] = 1;
}

//{"act":"sync","sID":22,"dID":255,"mID":0,"mt":457556}
int runID(int srcid, int destid, int idmask)
{
  
   int ret = -1;
   if ((srcid <= MAX_IDS)&&(destid <= MAX_IDS)) {
    
   
     if (destid == myID) {
      pingResponse(srcid);// ping response   
      return 0;
     }
     
     if (destid == MAX_IDS) {  // regular scan not a ping back
       baseTime[srcid] = millis() - (srcid * timeGap);
      
       if (accepted[srcid] == 0){   // new id found now we need to ping back
         pingTime = millis() + random(10);
         pingId = srcid;
         return 1;
       } 
      if (inIdMask(myID, idmask)) {
...
void setNextTime(int id)
{
  if (id < MAX_IDS) {
    baseTime[id] += sendWait + random(10);
  }
}
...
//
// this base time is really just for this run
//
void setBaseTime( int myid, int toid)
{
  
  if((toid< MAX_IDS) && (myid< MAX_IDS)) {
     baseCount[toid] ++;
     baseTime[toid] = millis() - (toid * timeGap);
     //nextTimetoSend =  baseTime[toid] +(myid * timeGap);
     //setNextTime();
  }
}
