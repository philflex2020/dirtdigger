...
void loop() {
  ...
  if (packetSize) {
    ...
    StaticJsonDocument<240> sdoc;
    DeserializationError error = deserializeJson(sdoc, packet);
    if (error) {
      ...
      counter = sdoc["counter"][0];
   }
    // Display Info
    Display.clearBuffer();  
    Display.setCursor(0,12); Display.print("LoRa Receiver");
    Display.setCursor(0,26); Display.print("Received counter:");
    Display.setCursor(0,42); Display.print(counter);
