...
void where() {
  ...
  output("c",AngularServos[ServoCuf].Angle);
  output("F",FeedRate);
...
void help() {
  ...
  Serial.println(F("M101 [S(Base Angle)]; - set angle of Base"));
  Serial.println(F("M102 [S(Left Angle)]; - set angle Left"));
  Serial.println(F("M103 [S(Right Angle)]; - set angle Right"));
  Serial.println(F("M104 [S(Wrist angle)]; - set angle of Wrist"));
  Serial.println(F("M105 [S(Cuf angle)]; - set angle of Cuf"));
...
int setpoint (double NewX, double NewY, double NewZ){
    double NewR, NewH, NewA;  Serial.println(F("M105 [S(Wrist angle)]; - set angle of Wrist"));
...
void processCommand() {
  ...
  case 105: {
    //Serial.print(parsenumber('S',90)); 
    ASet(AngularServos[ServoCuf], parsenumber('S',AngularServos[ServoCuf].Angle));
    break; // set Claw 
  }
  case 104: {
    //Serial.print(parsenumber('S',90)); 
  ...
  case 103: {
    //Serial.print(parsenumber('S',90)); 
    ASet(AngularServos[ServoRight], parsenumber('S',AngularServos[ServoRight].Angle));
    break; // set Base 
  }
  case 102: {
    //Serial.print(parsenumber('S',90)); 
    ASet(AngularServos[ServoLeft], parsenumber('S',AngularServos[ServoLeft].Angle));
    break; // set Base 
  }
  case 101: {
    //Serial.print(parsenumber('S',90)); 
    ASet(AngularServos[ServoBase], parsenumber('S',AngularServos[ServoBase].Angle));
    break; // set Base 
  }
...
void GCodeControl(){
  ...
  while(Serial.available() > 0) {  // if something is available
    ...
    //Serial.print("sofar");  // repeat it back so I know you got the message
    //Serial.println(sofar);  // repeat it back so I know you got the message
